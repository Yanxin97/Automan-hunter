 ...
 *
 * Created on: Dec 30, 2021 20:52
 * Description:
 *
 * Copyright (c) 2021 Yanxin Zhou
 */
...
class HunterMessenger {
 ...
 private:
  ...
  void TwistCmdCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
    if (!simulated_robot_) {
      SetHunterMotionCommand(hunter_, msg);
    } else {
  ...
  template <typename T,
            std::enable_if_t<std::is_base_of<HunterRobot, T>::value,
                             bool> = true>
  void SetHunterMotionCommand(std::shared_ptr<T> base,
  ...
  void PublishOdometryToROS(const MotionStateMessage &msg, double dt) {
    ...
    if (std::is_base_of<HunterRobot, HunterType>::value) {
